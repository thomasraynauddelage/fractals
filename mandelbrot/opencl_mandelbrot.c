...
For the OpenCL syntax I used the  hello worl example provided by apple from
...
To run:
./opencl_mandelbrot <c(cpu) or g(gpu)><size of work group> <output.ppm>
*/
...
int main(int argc, char *argv[]){
	...
	if (argc != 4) {
		fprintf(stderr, "The format should be ./opencl_mandelbrot <c(cpu) or g(gpu)><size of work group> <output.ppm>");
		return 1;
	}
