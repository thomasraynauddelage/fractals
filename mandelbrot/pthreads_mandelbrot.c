/*To compile:
 gcc -o pthreads_mandelbrot pthreads_mandelbrot.c -lpthread  
 To run:
./pthreads_mandelbrot <number of threads> <output.ppm>
*/
...
int parallelMandelbrot(int width, int height, double cx_min, double cx_max, double cy_min, double cy_max, int max_iterations, char* filename, int num_threads){
...
int main(int argc, char *argv[]){
	...
	if (argc != 3) {
		fprintf(stderr, "The format should be ./pthreads_mandelbrot <number of threads> <output.ppm>");
		return 1;
	}
  ...
  //Harcoded values, otherwise too many values to enter at the command prompt
	...
	gettimeofday(&start,NULL);
	for(i=0; i<100; i++){
		parallelMandelbrot(width, height, cx_min, cx_max, cy_min, cy_max, max_iterations, filename, num_threads);
		printf("iteration: %d\n",i);
	}
	gettimeofday(&end, NULL);
	//end timer
